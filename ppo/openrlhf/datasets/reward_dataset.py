...
def preprocess_data(
    ...
    if is_cls_temp:
        ...
        for k, v in labels.items():
            flag = flag or v
        margin = temperature if flag else 1.0
        # print(margin)
    elif is_scaled: 
        strength = float(data[label_key])
        margin = strength
...
class RewardDataset(Dataset):
    ...
    def __init__(
        ...
        self.is_cls_temp = (self.strategy.args.loss == "cls_temp")
    ...
    def process_data(self, data):
        prompt, chosen, reject, margin = preprocess_data(
            ...
            self.is_scaled,
            self.label_key,
            self.strategy.args.cls_temperature,
        )
    ...
    def __getitem__(self, idx):
        ...
        reject_token["input_ids"][0][-1] = self.tokenizer.eos_token_id[-1] if isinstance(self.tokenizer.eos_token_id, list) else self.tokenizer.eos_token_id
