...
def compute_approx_kl(
    ...
    Args:
        ...
        use_kl_estimator_k3: Whether to use the KL estimator k3.
        style: The style of the KL divergence. "token" or "response".
    """
    assert style in ["token", "response"], "KL divergence style not \"token\" or \"response\"."
...
def compute_reward(
    ...
    if action_mask is not None:
        ...
        reward = []
        if style == "response":
            reward = [(r[i] - kl_coef*kl[i]).unsqueeze(0) for i in range(len(kl))]
        else: 
