...
def _validate_args(args):
    ...
    if args.vllm_num_engines > 0:
        assert (
            actor_world_size >= args.vllm_num_engines
        ), f"actor_world_size must be greater than vllm_num_engines, got {actor_world_size} and {args.vllm_num_engines}"
...
def train(args):
    ...
    pg = None
    if (not args.disable_critic) and args.colocate_critic_reward:
        assert (
    ...
    if args.save_value_network and not args.disable_critic:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--policy_gradient_style", default="ppo", choices=["ppo", "grpo", "rloo", "grpo_resp", "rloo_resp", "vpg_resp", "rloo_resp_kl", "rloo_kl", "pmd"], help="Style for policy gradient estimate"
    )
    parser.add_argument("--critic_max_steps", type=int, default=100)
    parser.add_argument("--disable_critic", action="store_true", default=False, help="Disable critic model")
    ...
    parser.add_argument("--reward_model_type", type=str, default="reward", help="reward model or preference model")
    ...
    parser.add_argument("--context_key", type=str, default=None, help="context key")
    ...
    assert not (args.disable_critic and args.policy_gradient_style == "ppo"), "Policy gradient style cannot be \"ppo\" when critic model is disabled."
    assert not (args.policy_gradient_style in ["grpo", "rloo", "grpo_resp", "rloo_resp", "rloo_resp_kl", "rloo_kl"] and args.n_samples_per_prompt == 1), "When policy gradient style is \"grpo\" or \"rloo\", multiple samples per prompt is required."
    assert not (args.policy_gradient_style in ["grpo", "rloo", "grpo_resp", "rloo_resp", "rloo_resp_kl", "rloo_kl"] and args.lambd < 0.999), "[Temporary warning] When policy gradient style is \"grpo\" or \"rloo\", `--lambd` should be set to 1."
    assert args.micro_rollout_batch_size % args.n_samples_per_prompt == 0, "[Temporary warning] `--micro_rollout_batch_size` should be divisible by `--n_samples_per_prompt`."
