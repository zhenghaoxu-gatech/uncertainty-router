...
class ActorPPOTrainer(PPOTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.experience_maker = RemoteExperienceMaker(
            ...
            disable_critic=self.disable_critic,
        )
    ...
    def fit(
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                if episode < args.num_episodes // 2 and steps >= self.args.critic_max_steps:    # currently set upper limit
                    break
                ...
                if args.policy_gradient_style == "pmd": 
                    if episode < args.num_episodes // 2: 
                        train_critic = True
                        train_actor = False
                    else:
                        train_critic = False
                        train_actor = True
                        self.critic_train_remote = False
                else:
                    train_critic = True
                    train_actor = True
                status = self.ppo_train(steps, train_critic, train_actor)
    ...
    def ppo_train(self, global_steps, train_critic=True, train_actor=True):
        # 1. ensure all experience makers done
        self.experience_maker.flush()
        torch.distributed.barrier()

        # 2. triger remote critic model training
        if self.critic_train_remote and train_critic:
            critic_status_ref = self.critic.fit.remote()

        # 3. actor model training
        if global_steps > self.freezing_actor_steps and train_actor:
...
class ActorModelRayActor(BasePPORole):
    ...
    def fit(
        ...
        disable_critic = True if critic_model is None else False
    ...
    def save_model(self):
        ...
        if args.save_best: 
            ...
            _, states = self.strategy.load_ckpt(self.actor.model, ckpt_path, tag="best_model")
            best_global_step = states["best_global_step"]
            self.strategy.print(f"Reloaded checkpoint: {ckpt_path}, global step: {best_global_step}")
