...
from openrlhf.models import LogExpLoss, PairWiseLoss, ScaledPairWiseLoss, CenteredPairWiseLoss, AdaptivePairWiseLoss, TemperaturePairWiseLoss
...
class RewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        if loss == "sigmoid":
            ...
            self.loss_fn = CenteredPairWiseLoss(center_coef=self.strategy.args.center_coef)
            self.strategy.print(f"Centered LogSigmoid Loss with {self.strategy.args.center_coef}")
        elif loss == "aps":
            args = self.args
            self.loss_fn = AdaptivePairWiseLoss(rho=args.aps_rho, tau_max=args.aps_tau_max, mode=args.aps_mode)
            self.strategy.print(f"Adaptive Preference Scaling Loss with mode={args.aps_mode}, rho={args.aps_rho}, tau_max={args.aps_tau_max}")
        elif loss == "cls_temp":
            self.loss_fn = TemperaturePairWiseLoss()
            self.strategy.print("LogSigmoid Loss with Classified Temperature Scaling")
        elif loss == "scaled_bt": 
            self.loss_fn = ScaledPairWiseLoss()
            self.strategy.print("Scaled LogSigmoid Loss")
        else:
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                if self.margin_loss or self.args.loss == "scaled_bt":
                    margin = torch.tensor(margin).to(torch.cuda.current_device())
                elif self.args.loss == "cls_temp": 
                    temperature = torch.tensor(margin).to(torch.cuda.current_device())
                    margin = None
                else:
                    margin = None
                ...
                if self.args.loss == "aps":
                    preference_loss, tau = self.loss_fn(chosen_reward, reject_reward, margin)
                elif self.args.loss == "cls_temp":
                    preference_loss = self.loss_fn(chosen_reward, reject_reward, temperature)
                    tau = temperature
                else:
                ...
                if self.args.loss in ["aps", "cls_temp"]:
                    self.batch_tau.extend(tau.tolist())
                    # logs_dict["tau"] = tau.squeeze().tolist()
                if self.aux_loss:
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        if global_step % args.logging_steps == 0:
            ...
            if self._wandb is not None and self.strategy.is_rank_0():
                if self.args.loss in ["aps", "cls_temp"]:
                    logs_dict["tau"] = self._wandb.Histogram(self.batch_tau)
                    self.batch_tau.clear()
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                if self.args.loss == "aps":
                    loss, tau = self.loss_fn(chosen_reward, reject_reward, margin)
                elif self.args.loss == "cls_temp":
                    loss = self.loss_fn(chosen_reward, reject_reward, temperature)
                else:
