        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_reward, aux_loss = self.concatenated_forward(
                        self.model, chosen_ids, c_mask, last_epoch=(epoch==self.epochs-1)
                    )
                    if self.args.loss == "rdrop":
                        chosen_reward_, aux_loss_ = self.concatenated_forward(
                        self.model, chosen_ids, c_mask, last_epoch=(epoch==self.epochs-1)
                        )
                else:
                    ...
                    chosen_reward, aux_loss = self.packed_samples_forward(
                        self.model, packed_input_ids, packed_attention_masks, packed_seq_lens, last_epoch=(epoch==self.epochs-1)
                    )
                    if self.args.loss == "rdrop":
                ...
                if not (self.args.use_gp and (epoch==self.epochs-1)):
