...
def evaluate_on_rmbench(
    ...
    if local_rank == 0 or local_rank == -1:
        ...
        correlation = plot_accuracy_vs_uncertainty(
            total_diffs, 
            total_uncertainties, 
            save_path=os.path.join(save_path, 'accuracy_vs_uncertainty_rmbench.png')
        )
...
def plot_from_saved_results(results_dir: str):
    ...
    try:
        ...
        ece = plot_score_distributions(
            ...
            save_path=os.path.join(results_dir, 'score_distributions_rmbench_replot.png')
        )
        correlation = plot_accuracy_vs_uncertainty(
            final_diffs,
            final_uncertainties,
            save_path=os.path.join(results_dir, 'accuracy_vs_uncertainty_rmbench_replot.png')
        )
